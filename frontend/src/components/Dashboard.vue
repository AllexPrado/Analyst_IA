...
async function handleChat({ pergunta }) {
  ...
  chatResposta.value = ''
  try {
    const res = await axios.post('/api/chat', { pergunta }, { timeout: 60000 })
    
    if (res && res.data && typeof res.data === 'object') {
      chatResposta.value = res.data.resposta || 'Resposta vazia da IA.'
    } else {
      chatResposta.value = 'Erro: Resposta inválida da IA.'
    }
  } catch (error) {
    ...
    if (error.response && error.response.data) {
      if (typeof error.response.data === 'object' && error.response.data.mensagem) {
