<template>
  <div class="py-8">
    <div v-if="erroInsights" class="flex flex-col items-center justify-center h-96">
      <font-awesome-icon icon="exclamation-triangle" class="text-yellow-400 text-3xl mb-2" />
      <span class="text-yellow-300 text-lg">{{ mensagemErroInsights || 'Erro ao carregar dados de insights.' }}</span>
      <button @click="fetchInsights" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Tentar novamente</button>
    </div>
    <div v-else>
      <!-- Cabeçalho e filtros -->
...
const erroInsights = ref(false)
const mensagemErroInsights = ref('')
...
async function fetchInsights() {
  erroInsights.value = false
  mensagemErroInsights.value = ''
  try {
    const insights = await getInsights()
    if (insights && insights.erro) {
      erroInsights.value = true
      mensagemErroInsights.value = insights.mensagem || 'Nenhum dado real de insights disponível.'
    } else {
      dados.value = insights
    }
  } catch (e) {
    erroInsights.value = true
    mensagemErroInsights.value = 'Erro ao acessar o backend.'
  }
}
...
onMounted(async () => {
  ...
  if (dados.value && (!dados.value.resumoExecutivo || Object.keys(dados.value.resumoExecutivo).length === 0)) {
    dados.value.resumoExecutivo = generateExecutiveSummary()
  }
  
  // Atualiza gráficos se vierem do backend
  if (dados.value && dados.value.impactoSeries) {
    impactoSeries.value = dados.value.impactoSeries
  } else {
  ...
  if (dados.value && dados.value.problemasSeries) {
    problemasSeries.value = dados.value.problemasSeries
  } else {
    // Dados fallback para visualização
  ...
  if (dados.value && dados.value.impactoCategorias) {
    impactoOptions.xaxis.categories = dados.value.impactoCategorias
  } else {
    impactoOptions.xaxis.categories = ['Infraestrutura', 'Segurança', 'Desenvolvimento', 'Operações', 'Treinamento']
  }
  
  if (dados.value && dados.value.problemasCategorias) {
    problemasOptions.xaxis.categories = dados.value.problemasCategorias
  } else {
