...
const comparacao = ref('periodo-anterior')
...
function getKpiExecutiveSummary() {
  ...
  // Verificar se temos dados suficientes para gerar um resumo
  if (data.value.disponibilidade === null && data.value.taxaErro === null) {
    return "Não há dados suficientes para gerar um resumo executivo neste período.";
  }
  
  // Calcular a saúde geral do sistema com base nos indicadores disponíveis
  const saudeSistema = calcularSaudeSistema();
  
  // Gerar resumo com base na saúde do sistema
  if (saudeSistema > 80) {
    return `Os KPIs mostram um sistema com boa performance no período selecionado. ${getIndicadoresResumo()}`;
  } else if (saudeSistema > 60) {
    return `O sistema está operando com performance aceitável, mas com pontos de atenção. ${getIndicadoresResumo()}`;
  } else {
    return `Indicadores críticos detectados no período selecionado. ${getIndicadoresResumo()} Recomenda-se análise detalhada.`;
  }
}

function calcularSaudeSistema() {
  let pontos = 0;
  let total = 0;
  
  if (data.value.disponibilidade !== null) {
    pontos += data.value.disponibilidade;
    total += 100;
  }
  
  if (data.value.taxaErro !== null) {
    pontos += (100 - Math.min(data.value.taxaErro * 20, 100));
    total += 100;
  }
  
  if (data.value.latenciaMedia !== null) {
    // Considerar latência até 500ms como ótima (100 pontos)
    // e acima de 2000ms como ruim (0 pontos)
    const latenciaScore = Math.max(0, 100 - (data.value.latenciaMedia - 500) / 15);
...
function getIndicadoresResumo() {
  const resumos = [];
  
  if (data.value.disponibilidade !== null) {
    resumos.push(`Disponibilidade em ${data.value.disponibilidade}%`);
  }
  
  if (data.value.taxaErro !== null) {
    resumos.push(`Taxa de erro de ${data.value.taxaErro}%`);
  }
  
  if (data.value.latenciaMedia !== null) {
    resumos.push(`Latência média de ${data.value.latenciaMedia}ms`);
  }
  
  if (data.value.mttr !== null) {
    resumos.push(`MTTR de ${data.value.mttr} minutos`);
  }
...
function getMetricTrendIcon(metric, value) {
  // Se não houver valor ou for 0, retorna um ícone neutro
  if (value === null || value === undefined || value === 0) return 'minus';
  
  // Para métricas onde menor é melhor (erros, latência, MTTR)
  if (metric === 'erro' || metric === 'latencia' || metric === 'mttr' || metric === 'mttd') {
    return value < 0 ? 'arrow-down' : 'arrow-up';
  }
  
  // Para métricas onde maior é melhor (disponibilidade, throughput)
  return value > 0 ? 'arrow-up' : 'arrow-down';
}

function getMetricTrendClass(metric, value) {
  // Se não houver valor ou for 0, retorna uma classe neutra
